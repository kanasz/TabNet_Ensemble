...
if __name__ == '__main__':
    ...
    tuner = GaTuner(num_generations, num_parents, population, use_smote=False, clf_type=Classifier.WeightedSVC, numerical_cols=numerical_cols)
    tuner.run_experiment(data, 'results/SVC_WEIGHTED_yeast_3')
    print("--- total: %s seconds ---" % (time.time() - start_time))

    tuner = GaTuner(num_generations, num_parents, population, use_smote=True, clf_type=Classifier.SVC, numerical_cols=numerical_cols)
    tuner.run_experiment(data, 'results/SVC_SMOTE_yeast_3')
    print("--- total: %s seconds ---" % (time.time() - start_time))
    ...
    categorical_cols = None
    tuner = GaTuner(num_generations, num_parents, population, use_smote=False, clf_type=Classifier.WeightedSVC, numerical_cols=numerical_cols)
    tuner.run_experiment(data, 'results/SVC_WEIGHTED_yeast_4')
    print("--- total: %s seconds ---" % (time.time() - start_time))

    tuner = GaTuner(num_generations, num_parents, population, use_smote=True, clf_type=Classifier.SVC, numerical_cols=numerical_cols)
    tuner.run_experiment(data, 'results/SVC_SMOTE_yeast_4')
    print("--- total: %s seconds ---" % (time.time() - start_time))
    ...
    categorical_cols = None
    tuner = GaTuner(num_generations, num_parents, population, use_smote=False, clf_type=Classifier.WeightedSVC, numerical_cols=numerical_cols)
    tuner.run_experiment(data, 'results/SVC_WEIGHTED_yeast_5')
    print("--- total: %s seconds ---" % (time.time() - start_time))

    tuner = GaTuner(num_generations, num_parents, population, use_smote=True, clf_type=Classifier.SVC, numerical_cols=numerical_cols)
    tuner.run_experiment(data, 'results/SVC_SMOTE_yeast_5')
    print("--- total: %s seconds ---" % (time.time() - start_time))
    ...
    categorical_cols = None
    tuner = GaTuner(num_generations, num_parents, population, use_smote=False, clf_type=Classifier.WeightedSVC, numerical_cols=numerical_cols)
    tuner.run_experiment(data, 'results/SVC_WEIGHTED_yeast_6')
    print("--- total: %s seconds ---" % (time.time() - start_time))

    tuner = GaTuner(num_generations, num_parents, population, use_smote=True, clf_type=Classifier.SVC, numerical_cols=numerical_cols)
    tuner.run_experiment(data, 'results/SVC_SMOTE_yeast_6')
    print("--- total: %s seconds ---" % (time.time() - start_time))