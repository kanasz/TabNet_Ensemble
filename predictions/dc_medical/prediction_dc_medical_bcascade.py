...
if __name__ == '__main__':
    ...
    #threshold = 0.01  # keep services used by at least 1% of clients
    #service_cols = [col for col in data[0].columns if col.startswith('Service_') and '_Cum_' in col]
    #keep_cols = [col for col in service_cols if data[0][col].sum() / len(data[0]) >= threshold]


    tuner = GaTuner(num_generations, num_parents, population,
                    use_smote=False,
                    clf_type=Classifier.BalancedCascade, numerical_cols=numerical_cols)
    tuner.run_experiment(data,'results/BCascade_dc_medical')
