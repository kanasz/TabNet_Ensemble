...
def min_max(count):
    ...
    max_key = 0
    for i in count:
        if count[min_key] > count[i]:
            min_key = i
        if count[max_key] < count[i]:
            max_key = i
...
def Cluster(X_train, k, D, t, beta):
    ...
    absolute2 = torch.exp(torch.lgamma(stranger + 1)[0])
    v_d = absolute1 / absolute2
    d = D
    n = len(X_train)
    density = k / (n * v_d * pow(Radius_values, d)).to(device)

    CC = torch.tensor([-1 for _ in range(n)]).to(device)
    ...
    if len(cluster_no) > 1:
        ...
        # Mean calculations for each cluster
        a = []
...
def synthetic_generation(CC, X_train, Y_train, t):
    ...
    epsilon = 1
    X = torch.zeros(1, X_train.shape[1]).to(device)
...
def train(model, optimizer, criterion, train_loader, device=None):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        optimizer.step()
        prediction_labels_train = torch.cat(
            (prediction_labels_train, torch.tensor([x.argmax() for x in train_outputs.detach()]).to(device)), -1
        )
...
def test(ep, model, test_loader, device=None):
    ...
    for j, data in enumerate(test_loader, 0):
        ...
        prediction_labels_test = torch.cat(
            (prediction_labels_test, torch.tensor([x.argmax() for x in test_outputs.detach()]).to(device)), -1
        )
        groundtruth_labels_test = torch.cat((groundtruth_labels_test, test_labels), 0)
        all_probs.append(F.softmax(test_outputs.detach(), dim=1).cpu().numpy())
    ...
    f1_scores = sklearn.metrics.f1_score(gt, p, average='binary')
    gmean = imblearn.metrics.geometric_mean_score(gt.tolist(), p.tolist())
    probs = np.concatenate(all_probs, axis=0)
    auc = sklearn.metrics.roc_auc_score(gt, probs[:, 1])

    return balanced_test, mcc, f1_scores, gmean, auc