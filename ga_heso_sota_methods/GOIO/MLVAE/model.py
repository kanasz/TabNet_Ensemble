...
class Model_VAE(nn.Module):
    def __init__(self, num_layers, d_numerical, categories, d_token, n_head = 1, factor = 4,  bias = True, prototype_shape=[2,9,4]):
        ...
        self.proto = nn.Parameter(torch.randn(prototype_shape),
                                              requires_grad=True)
    ...
    def calc_sim_scores(self, z, labels):
        z_dist = F.normalize(z, dim=1)
        proto_dist = F.normalize(self.proto, dim=1)
        z_dist = rearrange(z_dist, 'a b c->a (b c)')
        proto_dist = rearrange(proto_dist, 'a b c->a (b c)')
        sim_scores = 0.0
        for i in torch.unique(labels):
            close = torch.cdist(z_dist[torch.where(labels==i)[0],:], proto_dist[i][None,:], p=2)
            away = torch.cdist(z_dist[torch.where(labels!=i)[0],:], proto_dist[i][None,:], p=2)
            sim_scores += torch.max(close.mean() - away.mean() + 0.5, torch.tensor(0, device=close.device))

        return sim_scores

    def forward(self, x_num, x_cat, labels):
        ...
        z = self.reparameterize(mu_z[:,1:,:], std_z[:,1:,:])

        dist_loss = self.calc_sim_scores(z, labels)


        return recon_x_num, recon_x_cat, mu_z, std_z, self.proto, dist_loss
