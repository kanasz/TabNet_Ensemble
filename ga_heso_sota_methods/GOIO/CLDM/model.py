...
Based on https://github.com/amazon-science/tabsyn/blob/main/tabsyn/model.py
"""
...
class MLPDiffusion(nn.Module):
    def __init__(self, d_in, dim_t = 512):
        ...
        self.proto_embed = nn.Sequential(
    ...
    def forward(self, x, noise_labels, class_labels=None):
        ...
        if class_labels is not None :
            emb=emb+self.proto_embed(class_labels)
...
class Precond(nn.Module):
    ...
    def forward(self, x, sigma, class_labels=None):
        ...
        x_in = c_in * x
        F_x = self.denoise_fn_F((x_in).to(dtype), c_noise.flatten(), class_labels=class_labels)
...
class Model(nn.Module):
    def __init__(self, denoise_fn, hid_dim, P_mean=-1.4, P_std=1.4, sigma_data=0.5, gamma=5, opts=None, pfgmpp = False):
        super().__init__()
    ...
    def forward(self, x, class_labels=None):
        ...
        loss, D_x, weight2 = self.loss_fn(self.denoise_fn_D, x, class_labels=class_labels)
        return loss.mean(-1).mean(), D_x, weight2
